...
void execute()
{
    ...
    // choosing
    // the desired option for
    // the user switch (option)
    {
    case 1:
        eligibleStude
            nts(s);
...
// current
// student roll number is same
as
    // the user input roll number if
    (s[i].rno == (a))
{
    // Update record at ith index
    // with (i + 1)th index for (j
    = i;
    j < n; j++)
    {
        strcpy(s[j].name, s[j +
...
record void add(struct student s[50])
{
